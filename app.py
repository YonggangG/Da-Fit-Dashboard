...
<body>
  <main class="wrap">
    ...
    <section class="detail-grid">
      <section class="panel heart-detail">
        <div class="chart-head"><h2 data-i18n="dailyHeartCurve">每日心率曲线</h2><select id="heartSelect" aria-label="选择心率日期"></select></div>
  ...
  <script>
    ...
    function applyLanguage(){document.documentElement.lang=lang==="en"?"en":"zh-CN"; document.getElementById("langSelect").value=lang; document.querySelectorAll("[data-i18n]").forEach(n=>n.textContent=tr(n.dataset.i18n)); document.querySelectorAll("[data-placeholder-key]").forEach(n=>n.placeholder=tr(n.dataset.placeholderKey)); renderSummary(); renderAdvice(); renderChart(); renderHeartOptions(); renderDailyHeart(); renderSleepOptions(); renderSleepDetail(); renderCalendar();}
    ...
    async function load(){ await initUser(); state=await fetch("/api/summary").then(r=>r.json()); renderMonthOptions(); renderSummary(); renderAdvice(); renderImports(); renderChart(); renderHeartOptions(); renderDailyHeart(); renderSleepOptions(); renderSleepDetail(); renderCalendar(); applyLanguage(); }
    ...
    function heartRows(){ return (state.series?.heart_rate||[]).filter(r=>heartSamples(r).length); }
    function renderHeartOptions(){ const rows=heartRows().filter(r=>sameMonth(r)); const sel=document.getElementById("heartSelect"); if(!rows.length){sel.innerHTML=`<option>${tr("noHeartCurve")}</option>`; selectedHeartMs=null; return} if(!selectedHeartMs||!rows.some(r=>String(r.date_ms)===String(selectedHeartMs)))selectedHeartMs=rows[rows.length-1].date_ms; sel.innerHTML=rows.slice().reverse().map(r=>{const values=heartSamples(r), valid=values.filter(v=>v>0).length; return `<option value="${r.date_ms}" ${String(r.date_ms)===String(selectedHeartMs)?"selected":""}>${dt(r.date_iso)} · ${valid}/${values.length}</option>`}).join(""); }
    ...
    document.getElementById("metricMonthSelect").addEventListener("change",(ev)=>{selectedMetricMonth=ev.target.value; selectedHeartMs=null; renderSummary(); renderAdvice(); renderChart(); renderHeartOptions(); renderDailyHeart();});
    document.getElementById("heartSelect").addEventListener("change",(ev)=>{selectedHeartMs=ev.target.value; renderDailyHeart();});
