...
<body>
  ...
  <script>
    ...
    function dailyReports(){ return (state.series?.daily_health||[]).filter(r=>r.raw_json); }
    ...
    function scoreGrade(score,grade){ const raw=String(grade||"").toUpperCase(); if(raw==="ATTENTION")return "Needs Improvement (ATTENTION)"; if(raw==="NORMAL")return "Normal"; if(raw)return raw.replaceAll("_"," "); if(score>=85)return "Excellent"; if(score>=70)return "Normal"; if(score>0)return "Needs Improvement"; return "--"; }
    ...
    function buildDailyReport(row){ const raw=rawDaily(row), s=state.series||{}, score=Number(raw.TOTAL_SCORE??row.total_score??0), grade=scoreGrade(score,raw.GRADE??row.grade), steps=Number(raw.STEPS_VALUE??row.steps??0), stepsGoal=Number(raw.STEPS_GOAL??userProfile.daily_steps_goal??10000), calories=Number(raw.CALORIES_VALUE??row.calories??0), caloriesGoal=Number(raw.CALORIES_GOAL??userProfile.daily_calories_goal??300), activity=Number(raw.ACTIVITY_VALUE??row.activity??0), activityGoal=Number(raw.ACTIVITY_GOAL??userProfile.daily_minutes_goal??30), sleep=Number(raw.SLEEP_DURATION_VALUE??0), sleepQ=Number(raw.SLEEP_QUALITY_VALUE??raw.SLEEP_QUALITY_RAW_VALUE??0), heart=Number(raw.HEART_RATE_VALUE??0), stress=Number(raw.STRESS_VALUE??0); const stepsAvg=avgText(reportRowsBefore(s.sport,row,r=>r.steps),"steps"), heartAvg=avgText(reportRowsBefore(s.heart_rate,row,r=>r.average),"bpm"), stressAvg=avgText(reportRowsBefore(s.stress,row,r=>r.average),""), sleepAvg=avgText(reportRowsBefore(s.daily_health,row,r=>Number(rawDaily(r).SLEEP_DURATION_VALUE||0)).map(v=>v/60),"h"); const sleepHours=sleep?`${Math.floor(sleep/60)}h${String(Math.round(sleep%60)).padStart(2,"0")}m`:"--"; const reportDate=msDate(raw.REPORT_DATE), displayDate=dt(row.date_iso), generated=raw.GENERATED_DATE?new Date(raw.GENERATED_DATE).toLocaleString():"--"; const metrics=[{key:"steps",level:Number(raw.STEPS_STATUS)===1?"warn":"good",title:lang==="en"?"Steps":"步数",value:`${fmt.format(steps)} / ${fmt.format(stepsGoal)}`,sub:stepsAvg+" · "+statusText(raw.STEPS_STATUS)},{key:"calories",level:calories>=caloriesGoal?"good":"warn",title:lang==="en"?"Calories":"消耗",value:`${num(calories,0)} / ${num(caloriesGoal,0)} kcal`,sub:statusText(raw.STEPS_STATUS)},{key:"activity",level:Number(raw.ACTIVITY_STATUS)===1?"warn":"good",title:lang==="en"?"Active minutes":"运动时长",value:`${num(activity,0)} / ${num(activityGoal,0)} min`,sub:statusText(raw.ACTIVITY_STATUS)},{key:"sleep",level:Number(raw.SLEEP_DURATION_STATUS)===1||sleep<420?"warn":"good",title:lang==="en"?"Sleep":"睡眠",value:sleepHours,sub:`${sleepAvg} · ${statusText(raw.SLEEP_DURATION_STATUS)} · quality ${num(sleepQ,0)}`},{key:"stress",level:Number(raw.STRESS_STATUS)===1?"warn":"good",title:lang==="en"?"Stress":"压力",value:num(stress,0),sub:`${stressAvg} · ${statusText(raw.STRESS_STATUS)}`},{key:"heart",level:Number(raw.HEART_RATE_STATUS)===1?"warn":"good",title:lang==="en"?"Heart rate":"心率",value:`${num(heart,0)} bpm`,sub:`${heartAvg} · ${statusText(raw.HEART_RATE_STATUS)}`}]; const focus=metrics.filter(m=>m.level==="warn").map(m=>m.title).slice(0,2).join(lang==="en"?", ":"、")|| (lang==="en"?"keep current routine":"继续保持当前节奏"); const narrative=lang==="en"?`Overall score ${num(score,0)} (${grade}). Activity was solid if steps, calories, or active minutes met target; the main items to watch are ${focus}.`:`综合分 ${num(score,0)}（${grade}）。活动量看步数、消耗和运动时长是否达标；今天主要需要关注的是 ${focus}。`; const lines=lang==="en"?[`DA FIT Daily Health Report ${displayDate}`,`Score: ${num(score,0)} (${grade})`,`Report day: ${reportDate}; generated: ${generated}`,...metrics.map(m=>`- ${m.title}: ${m.value}; ${m.sub}`),`Read: ${narrative}`]:[`DA FIT 每日健康日报 ${displayDate}`,`综合分：${num(score,0)}（${grade}）`,`统计日期：${reportDate}；生成时间：${generated}`,...metrics.map(m=>`- ${m.title}：${m.value}；${m.sub}`),`我的判断：${narrative}`]; return {score,grade,reportDate,displayDate,generated,metrics,narrative,text:lines.join("\n")}; }
    function renderDailyReport(){ const rows=dailyReports(), row=rows.find(r=>String(r.date_ms)===String(selectedReportMs))||rows.at(-1); if(!row){setText("reportScore","--"); setText("reportGrade","--"); setText("reportDate",tr("noDailyReport")); setText("reportTitle",tr("noDailyReport")); setText("reportNarrative","--"); document.getElementById("reportMetrics").innerHTML=""; document.getElementById("reportText").textContent=""; return} const report=buildDailyReport(row); setText("reportScore",num(report.score,0)); setText("reportGrade",report.grade); setText("reportDate",`${report.displayDate} · ${lang==="en"?"report day":"统计"} ${report.reportDate}`); setText("reportKicker",lang==="en"?"DA FIT Daily Report":"DA FIT Daily Report"); setText("reportTitle",lang==="en"?`Generated ${report.generated}`:`生成时间 ${report.generated}`); setText("reportNarrative",report.narrative); document.getElementById("reportMetrics").innerHTML=report.metrics.map(m=>`<div class="report-metric ${m.level}"><div class="report-metric-title">${escapeHtml(m.title)}</div><div class="report-metric-value">${escapeHtml(m.value)}</div><div class="report-metric-sub">${escapeHtml(m.sub)}</div></div>`).join(""); document.getElementById("reportText").textContent=report.text; }
